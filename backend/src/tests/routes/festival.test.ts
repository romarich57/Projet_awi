...
test('GET / - should return all festivals (200)', async () => {
    ...
    assert.ok(rows.some((f: any) => f.name === 'Festival 1'))
})
...
test('POST / - should create festival with valid data (201)', async () => {
    ...
    const { rows } = await pool.query(
     ...
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
        [newFestival.name, newFestival.start_date, newFestival.end_date,
...
test('POST / - should return created festival with id', async () => {
    const { rows } = await pool.query(
        `INSERT INTO festival (name, start_date, end_date, stock_tables_standard, stock_tables_grande, stock_tables_mairie, stock_chaises)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
        ['ID Test', new Date(), new Date(), 0, 0, 0, 0]
    )
...
test('PUT /:id - should update festival (200)', async () => {
    ...
    const { rowCount } = await pool.query(
        `UPDATE festival SET name = $1 WHERE id = $2`,
        ['Updated Name', festival.id]
    )
...
test('PUT /:id - should validate updated data', async () => {
    ...
    const { rows } = await pool.query(
        `UPDATE festival SET stock_chaises = $1 WHERE id = $2 RETURNING *`,
        [200, festival.id]
    )
