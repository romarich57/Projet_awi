...
import { createTestReservant, generateTestEmail, setupTests, teardownTests } from '../test-helpers.js';
/**
...
test('GET / - should sort by name ASC', async () => {
    ...
    await reservantRouter.stack
        ...
        ?.route?.stack[0]?.handle(mockReq, mockRes, () => { });
    assert.strictEqual(mockRes.statusCode, 200);
    const names = mockRes.jsonData.map((r) => r.name);
...
test('GET /:id - should return 404 if not found', async () => {
    ...
    await reservantRouter.stack
        .find((layer) => layer.route?.path === '/:id')
        ?.route?.stack[0]?.handle(mockReq, mockRes, () => { });
...
test('DELETE /:id - should verify actually deleted from DB', async () => {
    ...
    await reservantRouter.stack
        .find((layer) => layer.route?.path === '/:id' && layer.route?.methods?.delete)
        ?.route?.stack[0]?.handle(mockReqDelete, mockResDelete, () => { });
    // Try to get
    const mockReqGet = {
        params: { id: reservant.id }
    };
    ...
    await reservantRouter.stack
        .find((layer) => layer.route?.path === '/:id' && layer.route?.methods?.get)
        ?.route?.stack[0]?.handle(mockReqGet, mockResGet, () => { });
...
//# sourceMappingURL=reservant.test.js.map