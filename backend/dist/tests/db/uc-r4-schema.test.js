...
test('games table contient les colonnes UC-R4', async () => {
    const { rows } = await pool.query("SELECT column_name FROM information_schema.columns WHERE table_name = 'games'");
    const columns = rows.map((r) => r.column_name);
    ['min_players', 'max_players', 'prototype', 'duration_minutes', 'theme', 'description'].forEach((col) => assert.ok(columns.includes(col), `colonne ${col} manquante`));
...
//# sourceMappingURL=uc-r4-schema.test.js.map