...
router.get('/:id', async (req, res) => {
    const { id } = req.params;
    const { rows } = await pool.query('SELECT id, name, start_date, end_date, stock_tables_standard, stock_tables_grande, stock_tables_mairie, stock_chaises FROM festival WHERE id = $1', [id]);
    if (rows.length === 0) {
...
router.post('/', async (req, res) => {
    ...
    try {
        const { rows } = await pool.query(`INSERT INTO festival (name, stock_tables_standard, stock_tables_grande, stock_tables_mairie, stock_chaises, start_date, end_date)
             VALUES ($1, $2, $3, $4, $5, $6, $7)
             RETURNING id, name, stock_tables_standard, stock_tables_grande, stock_tables_mairie, stock_chaises, start_date, end_date`, [name, stock_tables_standard || 0, stock_tables_grande || 0, stock_tables_mairie || 0, stock_chaises || 0, start_date, end_date]);
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const { rowCount } = await pool.query(`UPDATE festival
             ...
             WHERE id = $8`, [name, stock_tables_standard, stock_tables_grande, stock_tables_mairie, stock_chaises, start_date, end_date, id]);
        if (rowCount === 0) {
            return res.status(404).json({ error: 'Festival non trouvé' });
        }
