...
function parseUpdate(body) {
    ...
    if (body.zone_plan_id !== undefined) {
        const value = body.zone_plan_id === null || body.zone_plan_id === '' ? null : Number(body.zone_plan_id);
...
router.patch('/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`UPDATE jeux_alloues SET ${sets.join(', ')} WHERE id = $${params.length + 1} RETURNING id`, [...params, allocationId]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: 'Allocation introuvable' });
        }
        const { rows } = await pool.query(`${allocationSelect} WHERE ja.id = $1
       GROUP BY
         ...
         g.description, g.image_url, g.rules_video_url`, [allocationId]);
        res.json(rows[0]);
    }
    catch (err) {
...
//# sourceMappingURL=allocatedGames.js.map