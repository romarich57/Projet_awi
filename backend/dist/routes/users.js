...
router.get('/me', async (req, res) => {
    ...
    try {
        const { rows } = await pool.query(`SELECT ${SAFE_FIELDS} FROM users WHERE id = $1 LIMIT 1`, [req.user.id]);
...
router.put('/me', async (req, res) => {
    ...
    const login = typeof req.body?.login === 'string' ? sanitize(req.body.login) : undefined;
    const firstName = typeof req.body?.firstName === 'string' ? sanitize(req.body.firstName) : undefined;
    const lastName = typeof req.body?.lastName === 'string' ? sanitize(req.body.lastName) : undefined;
...
router.put('/:id', requireAdmin, async (req, res) => {
    ...
    const login = typeof req.body?.login === 'string' ? sanitize(req.body.login) : undefined;
    const role = typeof req.body?.role === 'string' ? sanitize(req.body.role) : undefined;
    const firstName = typeof req.body?.firstName === 'string' ? sanitize(req.body.firstName) : undefined;
    const lastName = typeof req.body?.lastName === 'string' ? sanitize(req.body.lastName) : undefined;
    const email = typeof req.body?.email === 'string'
        ? sanitize(req.body.email).toLowerCase()
