...
router.get('/reservation/:reservationId', async (req, res) => {
    ...
    try {
        ...
        const reservationResult = await pool.query('SELECT workflow_id FROM reservation WHERE id = $1', [reservationId]);
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const { rows } = await pool.query(`UPDATE suivi_workflow 
            SET state = $1,
            ...
            RETURNING id, reservant_id, festival_id, state as etatcourant, 
                      liste_jeux_demandee, liste_jeux_obtenue, jeux_recus, presentera_jeux`, [state, liste_jeux_demandee, liste_jeux_obtenue, jeux_recus, presentera_jeux, id]);
        if (rows.length === 0) {
            return res.status(404).json({ error: 'Workflow non trouvé' });
        }
        res.json(rows[0]);
...
router.post('/:id/contact', async (req, res) => {
    ...
    try {
        ...
        const existingContact = await pool.query(`SELECT id FROM suivi_contact 
             WHERE workflow_id = $1 
             AND DATE(date_contact) = CURRENT_DATE`, [id]);
        if (existingContact.rows.length > 0) {
            return res.status(409).json({ error: 'Un contact a déjà été enregistré aujourd\'hui' });
        }
        // Insert into suivi_contact
        await pool.query(`INSERT INTO suivi_contact (workflow_id, date_contact) VALUES ($1, NOW())`, [id]);
        // Return updated list of dates
        const { rows } = await pool.query(`SELECT date_contact FROM suivi_contact WHERE workflow_id = $1 ORDER BY date_contact DESC`, [id]);
